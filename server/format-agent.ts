...
export class FormatAgent {
   ...
   */
  private async parseWithLLM(content: string, formatHint?: string): Promise<any[]> {
    ...
    try {
      ...
      try {
        ...
        if (parsedContent.type === 'complex_toggle' && 
          ...
          return [NotionBlocks.createComplexToggle(
            parsedContent.title,
   ...
   */
  private directBlockFormatting(content: string, formatHint?: string, hasMultipleLines: boolean = false): any[] {
   ...
   */
  private detectContentBlocks(content: string): any[] {
