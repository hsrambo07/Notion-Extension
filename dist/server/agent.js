...
export class NotionAgent {
     ...
     */
    async handleCreatePageAction(params) {
...
`;
     ...
     */
    async uploadFileToNotion(file, filename, maxRetries = 3) {
        let retryCount = 0;
        let lastError = null;
     ...
     */
    async createImageBlockFromUpload(file, filename, caption) {
        try {
     ...
     */
    async createFileBlockFromUpload(file, filename) {
        const { url, expiry_time } = await this.uploadFileToNotion(file, filename);
        return {
            object: "block",
            type: "file",
        ...
        try {
            ...
            if (file) {
                ...
                if (file.mimetype.startsWith('image/')) {
                    ...
                    block = await this.createImageBlockFromUpload(file.buffer, file.originalname, 
                ...
                if (sectionTitle && this.contextAwareHandler) {
                    ...
                    try {
                        const result = await this.contextAwareHandler.processCommand(`Add ${file.mimetype.startsWith('image/') ? 'image' : 'file'} "${file.originalname}" to ${sectionTitle} section in ${pageTitle}`);
                        if (result && result.success) {
                            return {
                                success: true,
     ...
     */
    async findSubpageId(parentId, subpageName) {
        try {
