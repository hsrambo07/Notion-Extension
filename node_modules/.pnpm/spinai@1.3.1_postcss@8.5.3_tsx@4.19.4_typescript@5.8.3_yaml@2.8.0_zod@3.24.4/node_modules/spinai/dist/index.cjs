...
function createGeminiLLM(config) {
  const client = new generativeAi.GoogleGenerativeAI(config.apiKey);
...
function createOpenAILLM(config) {
  const client = new OpenAI__default.default({ apiKey: config.apiKey });
...
function createAnthropicLLM(config) {
  ...
  const anthropic = new Anthropic__default.default({
...
function createBedrockLLM(config) {
  ...
  } : config.profile ? credentialProviders.fromIni({ profile: config.profile }) : void 0;
  const bedrock = new clientBedrockRuntime.BedrockRuntimeClient({
  ...
  return {
    ...
    async complete({
      ...
      const response = await bedrock.send(new clientBedrockRuntime.ConverseCommand({
        modelId: model,
...
async function runTaskLoop(params) {
  ...
  const sessionId = context.sessionId || uuid.v4();
...
exports.createAnthropicLLM = createAnthropicLLM;
exports.createBedrockLLM = createBedrockLLM;
exports.createCloudflareAILLM = createCloudflareAILLM;
exports.createDeepSeekAILLM = createDeepSeekAILLM;
exports.createGeminiLLM = createGeminiLLM;
exports.createHttpLLM = createHttpLLM;
exports.createOpenAILLM = createOpenAILLM;
exports.createXAILLM = createXAILLM;
